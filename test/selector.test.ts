import { querySelector, querySelectorAll } from "../src/selector.ts";
import { parse, render } from "../src/index.ts";
import { test, describe, it } from "jsr:@std/testing/bdd";
...
describe("type selector", () => {
	it("type", async () => {
		...
		const el = querySelector(parse(input), "h1");
	...
	it("compound type attribute = case sensitive", () => {
...
describe("pseudo-class", () => {
	...
	it("nth-child(3n)", async () => {
		const input =
			"<h1><span>a</span><span>b</span><span>c</span><span>d</span><span>e</span><span>f</span></h1>";
...
describe("components", () => {
	it("type", async () => {
		const input = "<SomeComponent>Hello world!</SomeComponent>";
		const el = querySelector(parse(input), "SomeComponent");
		expect(el).not.toBeNull();
		// biome-ignore lint/style/noNonNullAssertion: test assertion
		const output = await render(el!);
		expect(output).toEqual(input);
